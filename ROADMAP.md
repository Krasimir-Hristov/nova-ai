...
- [ ] Взимане на Google API ключ
- [ ] Инсталиране на `google-generativeai` Python библиотеката
- [ ] Съхранение на ключа в `.env` файл
- [ ] Обновяване на FastAPI ендпойнта за интеграция с Gemini
- [ ] Тестване на комуникацията с Gemini API
- [ ] Обработка на грешки при API извиквания
...
*Пример: Ако я попиташ "Кой е той?" след като сте говорили за Айнщайн, тя трябва да знае, че питаш за Айнщайн.*

- [ ] Инсталиране на `langchain` библиотеката
- [ ] Инсталиране на `langchain-community` пакет
- [ ] Внедряване на `ConversationBufferMemory` компонент
- [ ] Модифициране на FastAPI ендпойнта за пазене на историята
- [ ] Създаване на логика за управление на сесии
- [ ] Тестване на контекстуална памет в разговора
